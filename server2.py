...
while True:
    ...
    # 샤프닝 + 미디안
    image = cv2.imread('./conv/' + 'new_res.bmp', cv2.IMREAD_COLOR)
    sharpened_image = unsharp_mask(image)
    #sharpened_image.save('./conv/' + 'new_res_sharp+gaus.bmp','BMP')
    conv_dir = dir + "\\conv\\" + str(now) + ".bmp"
    cv2.imwrite(conv_dir, sharpened_image)
    # 미디안 필터
    blur = cv2.medianBlur(sharpened_image,5)
    conv_dir2 = dir + "\\conv\\" + "new_res_sharp+gaus+median" + str(now) + ".bmp"
    cv2.imwrite(conv_dir2, blur)
    #blur.save('./conv/' + 'new_res_sharp+gaus+median.bmp','BMP')
    
   #변환된 이미지 다시 불러오기
    #filename = ('./conv/after' + str(now) + '.jpg')
    filename2 = ('./conv/new_res_sharp+gaus+median' + str(now) + '.bmp')
    print('4point')
    f=open(filename2 ,'rb')
    data2=f.read()
    ####여기서 이미지 전송
